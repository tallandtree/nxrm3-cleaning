...
class ConfigReaderTest extends Specification {
    ...
    def "values are mapped correctly when reading the config file"() {
        ...
        then:
        readConfig.getConfigForAsset('team-z-docker-internal', 'mygrp/build-template-backend', '1.2.*').config ==
                expectedConfig.getConfigForAsset('team-z-docker-internal', 'mygrp/build-template-backend', '1.2.*').config
        readConfig.getConfigForAsset('team-z-docker-internal', 'mygrp/build-*', '1.3.*').config ==
                expectedConfig.getConfigForAsset('team-z-docker-internal', 'mygrp/build-*', '1.3.*').config
    }
}
