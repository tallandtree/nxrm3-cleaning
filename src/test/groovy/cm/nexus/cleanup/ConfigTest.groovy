...
class ConfigTest extends Specification {
    ...
    def "exact matching of items is favoured over regex matching"() {
        ...
        then:
        repoItems.config == [max_versions: 10, last_downloaded: [amount: 50, unit: 'day']]
        repoItems.key == '.*'
    }
    ...
    def "retrieving the config settings for an asset returns the default settings when there are no settings for repositories"() {
        ...
        then:
        repoItems.config == [max_versions: 5, last_downloaded: [amount: 30, unit: 'day']]
        repoItems.key == '.*'
    }
    ...
    def "retrieving the config settings for an asset returns the default settings overridden with the last_downloaded settings of the repository"() {
        ...
        then:
        repoItems.config == [max_versions: 5, last_downloaded: [amount: 40, unit: 'day']]
        repoItems.key == '.*'
    }
    ...
    def "retrieving the config settings for an asset returns the default settings overridden with the max_versions setting of the repository"() {
        ...
        then:
        repoItems.config == [max_versions: 10, last_downloaded: [amount: 30, unit: 'day']]
        repoItems.key == '.*'
    }
    ...
    def "retrieving the config settings for an asset returns the settings of the repository when there are no settings for components"() {
        ...
        then:
        repoItems.config == [max_versions: 10, last_downloaded: [amount: 40, unit: 'day']]
        repoItems.key == '.*'
    }
    ...
    def "retrieving the config settings for an asset returns the settings of the repository when there are no settings for the component"() {
        ...
        then:
        repoItems.config == [max_versions: 7, last_downloaded: [amount: 40, unit: 'day']]
        repoItems.key == '.*'
    }
    ...
    def "retrieving the config settings for an asset returns the settings of the component when there are no settings for assets"() {
        ...
        then:
        repoItems.config == [max_versions: 10, last_downloaded: [amount: 50, unit: 'week']]
        repoItems.key == '.*'
    }
    ...
    def "retrieving the config settings for an asset returns the settings of the repo overridden with the last_downloaded settings of the component"() {
        ...
        then:
        repoItems.config == [max_versions: 7, last_downloaded: [amount: 50, unit: 'week']]
        repoItems.key == '.*'
    }
    ...
    def "retrieving the config settings for an asset returns the default settings overridden with the last_downloaded settings of the component"() {
        ...
        then:
        repoItems.config == [max_versions: 5, last_downloaded: [amount: 50, unit: 'week']]
        repoItems.key == '.*'
    }
    ...
    def "retrieving the config settings for an asset returns the settings of the asset when present"() {
        ...
        then:
        repoItems.config == [max_versions: 10, last_downloaded: [amount: 60, unit: 'year']]
        repoItems.key == 'testasset'
    }
    ...
    def "retrieving the config settings for an asset returns the default settings overridden with the last_downloaded settings of the asset"() {
        ...
        then:
        repoItems.config == [max_versions: 5, last_downloaded: [amount: 60, unit: 'year']]
        repoItems.key == 'testasset'
    }
    ...
    def "retrieving the config settings for an asset returns the settings of the component overridden with the last_downloaded settings of the asset"() {
        ...
        then:
        repoItems.config == [max_versions: 7, last_downloaded: [amount: 60, unit: 'year']]
        repoItems.key == 'testasset'
    }
    ...
    def "retrieving the config settings for an asset returns the settings of the repo when the name of the repo matches a regex in the repo config settings"() {
        ...
        then:
        repoItems.config == [max_versions: 8, last_downloaded: [amount: 40, unit: 'day']]
        repoItems.key == '.*'
    }
    ...
    def "retrieving the config settings for an asset returns the settings of the repository when the name of the asset does not match a regex in the asset config settings although it matches with another component asset settings"() {
        given:
        def configItems = [default: [max_versions: 5, last_downloaded: [amount: 30, unit: 'day']], testrepo: [last_downloaded: [amount: 40, unit: 'day'], testcomponent2: [last_downloaded: [amount: 50, unit: 'week'], 'testas.*': [max_versions: 10, last_downloaded: [amount: 60, unit: 'year']]]]]
        def config = new Config(log, configItems)
    ...
    def "retrieving the config settings for an asset returns the default settings when the name of the asset does not match a regex in the asset config settings although it matches with another repository component asset settings"() {
        given:
        def configItems = [default: [max_versions: 5, last_downloaded: [amount: 30, unit: 'day']], testrepo2: [last_downloaded: [amount: 40, unit: 'day'], testcomponent: [last_downloaded: [amount: 50, unit: 'week'], 'testas.*': [max_versions: 10, last_downloaded: [amount: 60, unit: 'year']]]]]
        def config = new Config(log, configItems)
